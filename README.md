...
- **Feedback Invitations**: Sends a follow-up email on the first cron run at or after the appointment starts inviting the attendee to share feedback.
- **Calendar Invites**: Attaches industry-standard `.ics` calendar files to scheduled and canceled appointment emails so members and hosts can add or remove the event from their calendars. Slot selections are used first (for exact appointment duration), then timerange data is used as a fallback. The invite is a real `METHOD:REQUEST` invitation: the `ORGANIZER` is the address the mail is actually sent from (the SMTP module's `smtp_from` when that module is on, otherwise `email_sender`), and both participants are listed as `ATTENDEE` with `RSVP=TRUE;PARTSTAT=NEEDS-ACTION`. Gmail and Outlook only show Yes/No/Maybe and auto-add to the calendar when the organizer matches the sender, which is why `email_sender` alone is not used. The site repo carries a small patch to the SMTP module (`patches/smtp-inline-icalendar-part.patch`) that also emits the calendar as an inline `text/calendar` alternative part, which Outlook requires. Whether the event lands on a recipient's calendar without a click still depends on their own calendar setting ("Add invitations to my calendar").
...
- **Feedback Invitation**: On the first cron run at or after the scheduled start, the booking member receives one feedback invitation email. Canceled/unpublished appointments and appointments with written feedback are skipped. A recorded outcome alone does not suppress the member invitation. Date-only appointments retain next-calendar-day delivery. Failed sends retry for up to three days; the initial run includes only yesterday onward, and existing sent markers prevent repeat invitations. The email asks members to respond after finishing. Delivery timing depends on actual cron invocation frequency; an hourly cron can send about an hour after the start, and outages can delay it further. No Slack feedback messages are sent.
